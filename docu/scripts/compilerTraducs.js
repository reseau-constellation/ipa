...
const languePrincipale = "fr"
const langues = ["த", "es", "kaq", "ខ្មែរ", "हिं"]
const dossierTraductions = "traducs";
const racineProjet = "src"
...
const mettreFichiersTraducsÀJour = (dicTraducs) => {
    if (!fs.existsSync(dossierTraductions) || !fs.statSync(dossierTraductions).isDirectory()) {
        fs.mkdirSync(dossierTraductions, { recursive: true })
    }
    const fichierTraducs = path.join(dossierTraductions, languePrincipale + ".json")
    fs.writeFileSync(fichierTraducs, JSON.stringify(dicTraducs, null, 4));

    for (const langue of langues) {
        const fichierTraducsLangue = path.join(dossierTraductions, langue + ".json")
        const traducsLangue = fs.existsSync(fichierTraducsLangue) ? JSON.parse(fs.readFileSync(fichierTraducsLangue)) : {}
        fs.writeFileSync(fichierTraducsLangue, JSON.stringify(traducsLangue, null, 4))
    };
};
...
const compilerTraductions = () => {
    for (const langue of langues) {
        ...
        fichierTraducsLangue = path.join(dossierTraductions, langue + ".json")
        const traductions = fs.existsSync(fichierTraducsLangue) ? JSON.parse(fs.readFileSync(fichierTraducsLangue)) : {};
        ...
        for (const fichier of walk(racineProjet)) {
            ...
            const dossierSourceTraduite = fichierSourceTraduite.split(path.sep).slice(0, -1).join(path.sep)
            if (!fs.existsSync(dossierSourceTraduite)) fs.mkdirSync(dossierSourceTraduite, { recursive: true })
