...
import { changerLangue, déjàGénérés, ouvrirConstellation, optsPrises, type schémaFObtCheminImage } from "./utils.js";
...
export const prisesAccueil = async ({ 
    ...
    if (déjàGénérés({ images: ['pageAccueil.png', "changerLangue.png"], langue, obtCheminImage })) return;
    ...
    await page.screenshot({ path: obtCheminImage({ nomImage: 'pageAccueil.png', langue }), ...optsPrises });
    ...
    if (nomLangue) {
        ...
        await page.screenshot({ path: obtCheminImage({ nomImage: 'changerLangue.png', langue }), ...optsPrises });
    }
}