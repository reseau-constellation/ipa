...
export type schémaFObtCheminImage = ({ nomImage, langue }: {
  nomImage: string;
  langue: string;
})=>string

export const générerFObtCheminImage = ({ dossierRacine, dossierOriginales, dossierTraduites, langueSource }: {
  dossierRacine: string;
  dossierOriginales: string;
  dossierTraduites: string;
  langueSource: string;
}): schémaFObtCheminImage => {
  return ({ nomImage, langue }: { nomImage: string, langue: string }): string => {
...
export const déjàGénérés = ({images, langue, obtCheminImage}: {images: string[], langue: string, obtCheminImage: schémaFObtCheminImage }) => {
  const fichiers = images.map(nomImage => obtCheminImage({ nomImage, langue }));
...
export const changerLangue = async ({ langue, page }: { langue: string, page: Page }) => {
  ...
  if (nomLangue) {
    ...
    await new Promise<void>(résoudre => {
      ...
      page.getByText(nomLangue).click().then(()=>{
        clearTimeout(chrono);
        résoudre();
      });
    })
