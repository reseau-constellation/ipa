...
> {
  ...
  async démarrer(): Promise<{ oublier: Oublier }> {
    const oublier = await this.suivreBd({
      f: (autos) =>
        {const __id = uuidv(); console.log("suivi autos", __id, autos); this.queue.add(async () => await this.mettreAutosÀJour(autos)); console.log("suivi autos terminé", __id)},
    });
  ...
  async mettreAutosÀJour(
    ...
    for (const [id, auto] of Object.entries(autos)) {
      ...
      if (!valide(auto)) {
        ...
        journal.écrire({
          message: `Automatisation non valide : ${JSON.stringify(auto, undefined, 2)}\n${valide.errors?.map((e) => JSON.stringify(e, undefined, 2)).join(", ")}`,
        });
        continue;
      }
