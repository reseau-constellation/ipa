...
interface ComposantesNébuleuse {
  registrar: Registrar;
}

class Nébuleuse implements Startable {
  private readonly components: ComposantesNébuleuse;
  private topologyId?: string;

  constructor(components: ComposantesNébuleuse) {
...
export default function nébuleuse() {
  return (composantes: ComposantesNébuleuse) => {
    return new Nébuleuse(composantes);
  };
}
