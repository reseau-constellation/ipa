...
import { ServiceDonnéesNébuleuse, ServiceDonnéesNébuleuse, brancheBd } from "@/v2/crabe/services/services.js";
...
describe.only("Services Crabe", function () {
  ...
  describe("services données", function () {
    ...
    describe("accès données service", function () {
      ...
      class ServiceDonnéesTestA extends ServiceDonnéesNébuleuse<"A", StructureA> {
        constructor({ nébuleuse }: {nébuleuse: Nébuleuse<ServicesNécessairesCompte<ServicesLibp2pTest>>}) {
          super({
            clef: "A",
            nébuleuse,
            options: {
      ...
      class ServiceDonnéesTestB extends ServiceDonnéesNébuleuse<"B", StructureB> {
        constructor({ nébuleuse }: {nébuleuse: Nébuleuse<ServicesNécessairesCompte<ServicesLibp2pTest>>}) {
          super({
            clef: "B",
            nébuleuse,
      ...
      before(async () => {
        const {crabes, fermer} = await créerCrabesTest<Structure>({ n: 1, services: {
      ...
      it("suivi bd branche", async () => {
        ...
        await bdB.put("b/c", 1);
        const val = await obtenir(({siDéfini}) => bdB.suivreBd({ f: siDéfini() }));
