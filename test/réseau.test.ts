      ...
      describe("Réseau", function () {
        describe("Suivre en ligne", function () {
          ...
          it("Autres postes détectés", async () => {
            ...
            expect(val.map((r) => r.pair)).to.have.members([idsNodesSFIP[1], idsNodesSFIP[2]]);
          });
          ...
          it(
            ...
            async () => {
              ...
              expect(val.map((d) => d.infoDispositif.idOrbite)).to.have.members(idsOrbite);
            },
        ...
        describe("Membres fiables", function () {
          ...
          it("Faire confiance", async () => {
            ...
            const val = await fiablesPropres.attendreQue((x) => !!x && x.length > 0);
        ...
        describe("Membres bloqués", function () {
          ...
          it("On ne détecte pas le bloqué privé d'un autre membre", async () => {
            ...
            expect(val.map((b) => b.idBdCompte)).not.to.contain(idsBdCompte[2]);
          });
        ...
        describe("Suivre relations immédiates", function () {
          ...
          it("Enlever mot-clef détecté", async () => {
            await clients[0].motsClefs!.effacerMotClef({ idMotClef: idMotClef1 });
            const valPropres = await relationsPropres.attendreExiste();
            expect(valPropres.length).to.equal(0);
        ...
        describe("Suivre réplications", function () {
          ...
          it("Auteur de la BD pour commencer", async () => {
            ...
            expect(val.membres.map((m) => m.infoMembre.idBdCompte)).to.contain(
              idsBdCompte[0]
            );
          ...
          it(
            ...
            async () => {
              ...
              expect(val.membres.map((m) => m.infoMembre.idBdCompte)).to.have.members([idsBdCompte[0], idsBdCompte[1]]);
              expect(val.dispositifs.map((d) => d.idDispositif)).to.have.members([idsOrbite[0], idsOrbite[1]]);
            },
