...
describe("Connexions à moitié ouvertes", function () {
  ...
  const  connectéAuRelai = (pair: Libp2p<ServicesLibp2p>): Promise<void> => {
    const correspond = ({remoteAddr}: {remotePeer: PeerId, remoteAddr: Multiaddr}) => remoteAddr.toString().includes("relai-ws-libp2p.xn--rseau-constellation-bzb.ca")

    if (pair.getConnections().find(c=>correspond(c))) return Promise.resolve()
    
      return new Promise<void>(résoudre => {
        pair.addEventListener("connection:open", ({detail: {remoteAddr, remotePeer}}) => {
          if (correspond({remoteAddr, remotePeer}))
            résoudre()
        })
      })
  }

  const connectésParRelai = (pair1: Libp2p<ServicesLibp2p>, pair2: Libp2p<ServicesLibp2p>): Promise<void> => {
    const correspond = ({remotePeer, remoteAddr}: {remotePeer: PeerId, remoteAddr: Multiaddr}) => remotePeer.toString() === pair2.peerId.toString() && remoteAddr.toString().includes("p2p-circuit")
    
    if (pair1.getConnections().find(c=>correspond(c))) return Promise.resolve()
    
    return new Promise<void>(résoudre => {
      pair1.addEventListener("connection:open", ({detail: {remoteAddr, remotePeer}}) => {
        if (correspond({remoteAddr, remotePeer}))
          résoudre()
      })
    })
  ...
  const connectésParWebrtc = (pair1: Libp2p<ServicesLibp2p>, pair2: Libp2p<ServicesLibp2p>): Promise<PeerId> => {
    const correspond = ({remotePeer, remoteAddr}: {remotePeer: PeerId, remoteAddr: Multiaddr}) => remotePeer.toString() === pair2.peerId.toString() && remoteAddr.toString().startsWith("/webrtc")
    
    if (pair1.getConnections().find(c=>correspond(c))) return Promise.resolve(pair1.peerId)
    
    return new Promise<PeerId>(résoudre => {
      pair1.addEventListener("connection:open", ({detail: {remoteAddr, remotePeer}}) => {
        if (correspond({remoteAddr, remotePeer}))
          résoudre(pair1.peerId)
      })
    })
  };
  ...
  const abonnerFluxDirecte = async ({pair, protocole}: {pair: Libp2p<ServicesLibp2p>; protocole: string}): Promise<{
    stream: Stream;
    connection: Connection;
  }> => {
    await pair.register(protocole, {
      onConnect: (peer, connection) => {
          console.log(`sur pair ${pair.peerId.toString()} : pair ${peer.toString()} est souscrit à ${protocole} par connexion ${connection.remoteAddr.toString()}`)
        },
        onDisconnect: (peer: PeerId) => {
          console.log(`sur pair ${pair.peerId.toString()} : pair ${peer.toString()} n'est plus souscrit à ${protocole}`)
         }
    });
  ...
  const signalerFluxDirecte = async ({de, à, protocole}: {de: Libp2p<ServicesLibp2p>; à: Libp2p<ServicesLibp2p>; protocole: string}): Promise<{stream: Stream}> => {

    const retour = {stream: await pRetry(async () => await de.dialProtocol(à.peerId, protocole, { runOnLimitedConnection: true }))};

    de.addEventListener("connection:close", async ({detail : { remoteAddr, streams }})=>{
      ...
      if (streams.some(s=>s.protocol === protocole)){
        ...
        retour.stream = await pRetry(async ()=>await de.dialProtocol(à.peerId, protocole, { runOnLimitedConnection: true }));
  ...
  const vérifierGossipSub = async ({de, à, canal}: {de: Libp2p<ServicesLibp2p>, à: Libp2p<ServicesLibp2p>, canal: string}): Promise<void> => {
    const message = "message test " + uuidv4();
    const promesseReçu = new Promise<void>(résoudre => {
      à.services.pubsub.addEventListener("message", ({ detail: {topic, data}})=>{
    ...
    const intervale = setInterval(async ()=>{
      await de.services.pubsub.publish(canal, new TextEncoder().encode(message));
    }, 500)
    return promesseReçu
  };
  
  const vérifierFluxDirecte = ({fluxExpéditeur, fluxDestinataire}: {fluxExpéditeur: Stream, fluxDestinataire: Stream}) => {
  ...
  it("Communication GossipSub sur le relai", async () => {
    await Promise.all([
      vérifierGossipSub({de: sfip.libp2p, à: sfip2.libp2p, canal: CANAL_TEST}),
      vérifierGossipSub({de: sfip2.libp2p, à: sfip.libp2p, canal: CANAL_TEST})
    ]);
  });

  it("Abonner aux flux directes", async () => {
    const promesseFluxSortie1 = abonnerFluxDirecte({pair: sfip.libp2p, protocole: PROTOCOLE_TEST})
    const promesseFluxSortie2 = abonnerFluxDirecte({pair: sfip2.libp2p, protocole: PROTOCOLE_TEST})
    fluxEntrée1 = await signalerFluxDirecte({de: sfip.libp2p, à: sfip2.libp2p, protocole: PROTOCOLE_TEST})
    fluxEntrée2 = await signalerFluxDirecte({de: sfip2.libp2p, à: sfip.libp2p, protocole: PROTOCOLE_TEST})
  ...
  it("Communication flux directes sur le relai", async () => {
    await vérifierFluxDirecte({ fluxExpéditeur: fluxEntrée1.stream, fluxDestinataire: fluxSortie2.stream })
    console.log("Flux directe fonctionne sur relai 1 -> 2");
    await vérifierFluxDirecte({ fluxExpéditeur: fluxEntrée2.stream, fluxDestinataire: fluxSortie1.stream })
  ...
  it("Transition à WebRTC", async () => {
    ...
    console.log(`connectés par webrtc ${[sfip.libp2p.peerId, sfip2.libp2p.peerId].indexOf(pairWebRTC) + 1}`)
  });
