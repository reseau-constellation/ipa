...
describe("Profil", function () {
  ...
  describe("Courriels", function () {
    ...
    it("Pas de courriel pour commencer", async () => {
      const courriel = await await obtenir(({ siDéfini }) =>
        client.profil.suivreCourriel({
          f: siDéfini(),
        }),
      );
  ...
  describe("Noms", function () {
    ...
    it("Ajouter un nom", async () => {
      ...
      const nomsMaintenant = await obtenir<TraducsNom>(({ siPasVide }) =>
        client.profil.suivreNoms({
          f: siPasVide(),
        }),
      );
  ...
  describe("Bios", function () {
    it("Pas de bios pour commencer", async () => {
      const bios = await obtenir<TraducsNom>(({ siDéfini }) =>
        client.profil.suivreBios({
          f: siDéfini(),
        }),
      );
    ...
    it("Ajouter une bio", async () => {
      ...
      const bios = await obtenir<TraducsNom>(({ siPasVide }) =>
        client.profil.suivreBios({
          f: siPasVide(),
        }),
      );
      ...
      const bios2 = await obtenir<TraducsNom>(({ si }) =>
        client.profil.suivreBios({
          f: si((x) => Object.keys(x).length > 1),
        }),
      );
    ...
    it("Changer une bio", async () => {
      ...
      const bios = await obtenir<TraducsNom>(({ si }) =>
        client.profil.suivreBios({
  ...
  describe("Images", function () {
    ...
    it("Pas d'image pour commencer", async () => {
      const image = await obtenir(({ siDéfini }) =>
        client.profil.suivreImage({
          f: siDéfini(),
        }),
      );
