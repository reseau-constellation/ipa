...
describe("Variables", () => {
  ...
  beforeAll(async () => {
    ({ clients: constls, fOublier } = await constlTest.créerConstellationsTest({
      n: 1,
      créerConstellation,
    }));
  ...
  bench({
    ...
    beforeEach: async () =>
      await constls[0].variables.créerVariable({ catégorie: "numérique" }),
    fn: async (idVariable: string) => {
      await constls[0].variables.sauvegarderNomVariable({
        idVariable,
        nom: "Précipitation",
        langue: "fra",
      });
